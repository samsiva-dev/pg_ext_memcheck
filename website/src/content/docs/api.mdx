...
Opens a manual test window targeting a specific extension by context-name pattern. The monitoring mode (`all` / `executor` / `none`) is controlled separately by the `pg_ext_memcheck.memcheck_mode` GUC — `begin()` does **not** change the mode.
...
Closes the current manual test window by setting `pg_ext_memcheck.memcheck_mode` to `'none'` and returns violations belonging to the current session as a result set.
