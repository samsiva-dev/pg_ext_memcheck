...
| `ext_track_shmem` | Whether shmem sentinel checks are active this session. |
| `ext_track_dsm` | Whether DSM leak checks are active this session. |

`analyze_and_log_diff()` calls `ctx_matches_target()` and skips any diff entry whose context name does not match the pattern. `check_wrong_context_alloc()` calls `is_allowed_context_target()` in both passes and skips allowlisted contexts.
