...
1. Allocates `8KB` for each query in a child content under 'MessageContext' in the Planner Hook but never frees it → should trigger a `context_leak` violation.
...
3. It does only allocates and return. No other logic is present, so it won't cause any functional test failures — just memory leaks.
...
Now you can try with memcheck_mode set to `executor` to only capture executor-phase violations, or `none` to disable automatic hooks and use the manual `begin()` / `end()` API for more fine-grained control over the test window. You can also test with other workloads or scenarios like `tx_abort_loop`, and inspect the violation log to understand the memory behavior of your extension under different conditions. 
