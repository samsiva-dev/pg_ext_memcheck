...
4. **`context_leak WARNING` — `ExprContext` grew by ~194 KB**  
   An internal PostgreSQL expression evaluation context that accumulated state across the 1000 iterations of `SELECT count(*) FROM pg_class`. This is not a bug in the extension — it is normal PostgreSQL behavior under repeated execution within a single session. It surfaces here because the before-snapshot was taken before the scenario loop started and the after-snapshot is taken at `end()`.
