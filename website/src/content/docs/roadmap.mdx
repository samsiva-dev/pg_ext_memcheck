...
- **BGWorker crash harness** ✓ — `use_after_reset` and `oom_simulation` scenarios run in an isolated background worker; crash is confirmed via exit code, not by propagating SIGSEGV to the test session
