...
<Aside type="note" title="Availability">
  `growth_benchmark`, `tx_abort_loop`, and `shmem_sentinel_probe` are Phase 1 scenarios available now. `wrong_context_probe` is a Phase 2 scenario that is now implemented and available. The remaining Phase 2 scenarios (`context_reset_storm`, `concurrent_backends`, `dsm_lifecycle_check`) are not yet implemented.
</Aside>
...
**How it works:** `shmem_probe.c` allocates a `ProbeRegistry` in shared memory. The scenario calls `probe_register()` for the `ViolationLog` and `DsmTrackerState` segments — both of which were reserved with an extra byte in `_PG_init`. After the workload loop, `probe_check_all()` reads each sentinel byte. Any value other than `0xDE` logs a `shmem_overrun` ERROR violation.
