...
<Aside type="note" title="Availability">
  `growth_benchmark`, `tx_abort_loop`, and `shmem_sentinel_probe` are available now. All other scenarios (`context_reset_storm`, `concurrent_backends`, `dsm_lifecycle_check`, `wrong_context_probe`) are planned for a future phase and are not yet implemented.
</Aside>
