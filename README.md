...
- [ ] **SQL API** (`sql/pg_ext_memcheck--0.0.1.sql`): schema `ext_memcheck`, table `ext_memcheck.violation_log`, and `ext_memcheck.flush_violations()` implemented. Still needed: `begin()` / `end()` session-control functions and the `violations` view backed by `violation_log_read_all()`.
